#!/usr/bin/env python3
"""Run a reproducible colleague replay verification pass.
...
invariants in a report that a colleague or reviewer can inspect.
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# Repository Replay Verification Report",
        "",
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-dir",
        default=str(ROOT / "outputs" / "colleague-replay"),
    ...
    expected = args.expected_commit.strip() if args.expected_commit else head
    commit_match = (
        bool(head)
        and bool(expected)
    ...
    json_path = out_dir / f"colleague-replay-{stamp}.json"
    markdown_path = out_dir / f"colleague-replay-{stamp}.md"
    ...
    print(f"COLLEAGUE REPLAY: {automated_status}")
    print(f"HEAD: {head}")
